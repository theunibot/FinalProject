...
public abstract class CommandInterface {
	 ...
	 *
	 * @param cabinetType of cabinet at start; should be updated to reflect type
	 *                    at end
	 *
	...
	/**
	 * By default, commands will not execute if there is an error outstanding.
	 * If a command should execute even when there are errors, if should
	...
	/**
	 * By default, once an error occurs it will remain set even if some commands
	 * (those that override ignoreErrors) are successful. Only commands that
	 * support successClearsError (overrides it and returns true) will actually
	...
	/**
	 * By default commands will have various messages logged about their
	 * progress. Commands can override this and return false to avoid having
	 * their command operations logged.
	 *
	...
	/**
	 * Set the queue index for this command; used to track which queue this
	 * command is executing in
	 *
...
//            }
	...
	/**
	 * Internal function to move the arm to a position, while tracking it for
	 * future moves
	 *
	 * @param args      Command arguments to track the position of the arm
	 * @param toCabinet Cabinet to move the disc to
